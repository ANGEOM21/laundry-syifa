<div class="topbar-user-edit">

	<div class="topbar-user-container">

		<div class="topbar-back d-flex justify-content-center align-items-center">

			<a href="<?= BASEURL ?>" class="d-flex align-items-center">

				<svg xmlns="http://www.w3.org/2000/svg" width="20" height="30" viewBox="0 0 20 28" fill="none">

					<path d="M19 24.71L7.25641 14L19 3.29L15.3846 0L0 14L15.3846 28L19 24.71Z" fill="#fdfaf6" />
		...
		<li class="nav-item dropdown no-arrow" style="list-style: none;">
			...
			<a class="nav-link d-flex align-items-center p-0" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				...
				<div class="img-profile-navbar d-flex align-items-center justify-content-center">
					...
					<img class="rounded-circle" src="<?= BASEURL ?>/img/users/profile/<?= $data['user']['img_tmuld'] ?>" alt="" width="45">
...
<div class="notify-alert-box">
	...
	<p><?= $data['owner'] ?> ingin Mengirim Anda notifikasi pemrosessan Laundry anda</p>
...
<script>
	...
	function showNotification(pesan, prosess) {

		var notificationBody = new Notification("<?= $data['user']['name'] ?>", {

			icon: "<?= BASEURL ?>/img/icon/icon.svg",

			body: "Pesanan " + pesan + " anda " + prosess,
		...
		notificationBody.onclick = function() {
			...
			window.open("<?= BASEURL ?>/user/proses", "_blank");

			notificationBody.close(); // Tutup notifikasi setelah user mengkliknya
	...
	var pusher = new Pusher('2c347829e0890c4647df', {
	...
	var jumlahNotifikasi = $(".topbar-user-edit #alertsDropdown .dropdown-item:last").length;

	$(".topbar-user-edit #alertsDropdown .badge").html(jumlahNotifikasi);
	...
	channel.bind('my-event', function(data) {
		...
		if (<?= $data['user']['id_tmuld'] ?> == data.id) {

			$(".topbar-user-edit .dropdown-header").after(

				`<a class="dropdown-item d-flex align-items-center" href="/user/proses">
...
<span class="font-weight-bold">Pesanan ${data.pesanan} anda  ${data.prosess}</span>
			...
			$(".topbar-user-edit .dropdown-list p").html("Now Notification");

			jumlahNotifikasi += 1;

			$(".topbar-user-edit #alertsDropdown .badge").html(`${jumlahNotifikasi} +`);

			showNotification(data.pesanan, data.prosess);
